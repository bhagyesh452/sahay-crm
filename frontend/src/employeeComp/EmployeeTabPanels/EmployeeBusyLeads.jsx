...
function EmployeeBusyLeads({
    ...
    useEffect(() => {
        const fetchEmployeeData = async () => {
            ...
            const companyNumbers = busyData?.map(
            ...
            if (companyNumbers.length > 0) {
                ...
                try {
                    ...
                    data?.result.forEach((call) => {
                        ...
                        const matchedCompany = busyData.find((company) => {
                    ...
                    const updatedGeneralLeads = busyData.map((company) => {
                    ...
                    setBusyData(updatedGeneralLeads); // Update with enriched data
                } catch (err) {
                    console.log(err);
                } finally {
                    // setIsLoading(false);
                        ...
                        <table className="table table-vcenter table-nowrap" style={{ width: "1800px" }}>
                                            ...
                                            <th className='AEP-sticky-left-1'>
                                                <label className='table-check'>
                                                    <input
                                                        type="checkbox"
                                                        checked={
                                                            selectedRows && busyData && (selectedRows.length === busyData.length)
                                                        }
                            ...
                            </thead>
                            {
                                isLoading ? (
                                    ...
                                    <tbody>
                                        {busyData && busyData.map((company, index) => (
                                            <tr key={company._id}
                                                onMouseDown={() =>
                            ...
                            {busyData && busyData.length === 0 && !isLoading && (
                                <tbody>
                                    <tr>
...
export default EmployeeBusyLeads;