...
function EmployeeForwardedReport() {
  ...
  const handleFilterFollowDataToday = (isBdm) => {
    ...
    const filterFollowDataForwarded = isBdm ? followData.filter((company) => company.estPaymentDate === today && company.bdmName !== data.ename && company.caseType === "Forwarded") : followDataToday.filter((company) => company.caseType === "Forwarded")
  ...
  }, [selectedMonthOption, moreEmpData]);
        ...
        <div className="dash-card-head d-flex align-items-center justify-content-between">
          <h2 className="m-0">
            <select class="head-select form-select" id="head-select">
              ...
              <option value="2" disabled={!data.bdmWork} >Received as BDM</option>
            </select>
          </h2>
