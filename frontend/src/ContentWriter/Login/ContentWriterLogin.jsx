...
function ContentWriterLogin({ setContentWriterToken }) {
    ...
    const handleSendOtp = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${secretKey}/verifyCredentials`, {
                email,
                password,
                designations: ["Content Writer"], // Send array of designations
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const loginResponse = await axios.post(`${secretKey}/contentwriterlogin`, {
                email,
                password,
                designation,
            });

            const { contentwriterToken, userId } = loginResponse.data;
            setContentWriterToken(contentwriterToken);
            setUserId(userId);
            localStorage.setItem("contentWriterToken", contentwriterToken);
            localStorage.setItem("contentWriterUserId", userId);
            localStorage.setItem("contentWriterName", ename);
            navigate(`/content-writer/dashboard/${userId}`);
        } catch (error) {
