...
const DscPortalCharges = ({ companyName, serviceName, dscPortalCharges ,refreshData , mainStatus}) => {
    const [charges, setCharges] = useState(dscPortalCharges);
    const secretKey = process.env.REACT_APP_SECRET_KEY;
    const [openEmailPopup, setOpenEmailPopup] = useState(false);

    const handleSubmitPortalCharges = async () => {
        try {
            const response = await axios.post(`${secretKey}/rm-services/post-save-portalcharges-adminexecutive`, {
                companyName,
                serviceName,
                charges
            });
    ...
    return (
        <div>
            ...
            <div
                ...
                title={dscPortalCharges}
            >
                {!dscPortalCharges ? "Enter Portal Charges" : dscPortalCharges}
            </div>
