...
const DscEmailId = ({ companyName, serviceName, dscEmailId ,refreshData , mainStatus}) => {
    const [charges, setCharges] = useState(dscEmailId);
    const secretKey = process.env.REACT_APP_SECRET_KEY;
    const [openEmailPopup, setOpenEmailPopup] = useState(false);

    const handleSubmitPortalCharges = async () => {
        try {
            const response = await axios.post(`${secretKey}/rm-services/post-save-dscemailid-adminexecutive`, {
                companyName,
                serviceName,
                charges
            });
