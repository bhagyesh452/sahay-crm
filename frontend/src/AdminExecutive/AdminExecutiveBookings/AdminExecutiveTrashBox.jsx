...
function AdminExecutiveTrashBox({ setOpenTrashBox }) {
    ...
    //             })
    //             .sort((a, b) => {
    //                 const dateA = new Date(a.displayOfDateForAdminExecutive);
    //                 const dateB = new Date(b.displayOfDateForAdminExecutive);
    ...
    const handleDisplayOnToRm = async (companyName, company) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.post(`${secretKey}/rm-services/postmethodtogetbackfromtrashboxadminexecutive/${companyName}`);
                    // Handle the response
    ...
    const handleClearAllFromRmCertTrashbox = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.post(`${secretKey}/rm-services/postmethodtoremovelcompaniesfromtrashboxpemanentlyadminexecutive/` ,{
                        permanentlDeleteDateFromAdminExecutive:new Date()
                    });
                    // Handle the response
                    if (response.status === 200) {
                        fetchRedesignedFormData();
...
export default AdminExecutiveTrashBox;