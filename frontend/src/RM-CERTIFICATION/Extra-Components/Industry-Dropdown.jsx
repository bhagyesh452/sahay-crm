...
const IndustryDropdown = ({
    mainStatus, industry, setNewSubStatus, companyName, serviceName, refreshData, onIndustryChange , enableStatus
}) => {
    const [status, setStatus] = useState(industry || "");
    const [statusClass, setStatusClass] = useState("created-status");
    const [options, setOptions] = useState([]);
    ...
    const handleDisableIndustry = async () => {
        try {
            const response = await axios.post(`${secretKey}/rm-services/post-enable-industry`, {
                companyName,
                serviceName,
            ...
            refreshData();
            setIsEditing(true); // Enable editing mode after pencil click
        } catch (error) {
            console.log("Enable Update Status Industry", error.message);
        }
    };
    ...
    const handleSaveIndustry = async () => {
        try {
            ...
            console.log("Error Saving Industry", error.message);
        }
    };
    ...
    return (
        <div className="d-flex align-items-center justify-content-between">
            ...
            {mainStatus === "Process" && (
                isEditing ? (
                    <button className='td_add_remarks_btn ml-1' onClick={handleSaveIndustry}>
                        <FaSave />
                    </button>
                ) : (
                    <button className='td_add_remarks_btn ml-1' onClick={handleDisableIndustry}>
                        <FaPencilAlt />
                    </button>
                )
            )}
