...
function Received_booking_box() {
    ...
    try {
      ...
      const filteredAndSortedData = data
        .filter((obj) => {
          ...
          const hasValidMoreBookingsDate =
            obj.moreBookings &&
          ...
          const isDateValid =
            mainBookingDate >= today || hasValidMoreBookingsDate;

          // Return true if date is valid and visible to RM
          return (
            isDateValid &&
            obj.isVisibleToRmOfCerification !== false &&
            obj.permanentlDeleteFromRmCert !== true
          );
        })
        .sort((a, b) => {
      ...
      const processedData = filteredAndSortedData.map((item) => {
        ...
        const combinedServices = [
          ...
          ...(item.moreBookings || []).flatMap(
            (booking) => booking.servicesTakenByRmOfCertification || []
          ),
        ];
      ...
      const filteredServicesData = filteredAndSortedData.map((item) => {
        ...
        // Combine services
        const combinedServices = [...primaryServices, ...moreBookingServices];
      ...
      const nonMatchingCompanies = processedData.filter((item, index) => {
        ...
        const noCertificationServices = !(
          (item.servicesTakenByRmOfCertification &&
            item.servicesTakenByRmOfCertification.length > 0) ||
          (item.moreBookings &&
            item.moreBookings.some(
              (booking) =>
                booking.servicesTakenByRmOfCertification &&
                booking.servicesTakenByRmOfCertification.length > 0
            ))
        );
        ...
        return (
          !(
            item.combinedServices.length === filteredServiceNames.length &&
            item.combinedServices.every((service) =>
              filteredServiceNames.includes(service)
            )
          ) || noCertificationServices
        );
      });

      const completeData = response.data.sort((a, b) => {
        const dateA = new Date(a.lastActionDate);
        const dateB = new Date(b.lastActionDate);
        return dateB - dateA; // Sort in descending order
      });
  ...
  const [rmSelectedServiceMainBooking, setrmSelectedServiceMainBooking] =
    useState([]);
  const [rmSelectedServiceMoreBooking, setRmSelectedServiceMoreBooking] =
    useState([]);
  ...
  const handleOpenServices = (companyName) => {
    ...
    const selectedServicesHere = redesignedData
      ...
      .flatMap((company) => {
        ...
        const filteredServices = allServices.filter((service) => {
          const isServiceTakenByRmOfCertification =
            company.servicesTakenByRmOfCertification?.includes(
              service.serviceName
            );
          const isServiceTakenInMoreBookings = company.moreBookings.some(
            (booking) =>
          ...
          return (
            !isServiceTakenByRmOfCertification && !isServiceTakenInMoreBookings
          );
        });
    ...
    const servicesNames = selectedServicesHere.map(
      (service) => service.serviceName
    );
    setServiceNames(servicesNames);

    // Check if all service names are included in certificationLabelsNew
    const allServicesInCertificationLabelsNew = servicesNames.every(
      (serviceName) => certificationLabelsNew.includes(serviceName)
    );
  ...
  const handleSubmitServicesToSwap = async () => {
    ...
    if (!selectedCompanyData) {
      console.error(
        `Company with name '${selectedCompanyName}' not found in mainDataSwap.`
      );
      return;
    }
    ...
    const moreBookingServices = moreBookings.flatMap(
      (item) => item.services || []
    );
    ...
    const primaryServiceNames = filteredPrimaryServices.map(
      (service) => service.serviceName
    );
    const moreBookingServiceNames = filteredMoreBookingServices.map(
      (service) => service.serviceName
    );
    ...
    selectedServices.forEach((serviceName) => {
      const serviceData = combinedServices.find(
        (service) => service.serviceName === serviceName
      );
      // console.log("Service data :", serviceData);

      const remainingPaymentData = combinedRemainingpaymentsForServices.filter(
        (service) => service.serviceName === serviceName
      );

      const totalReceivedPayment = remainingPaymentData.reduce(
        (total, service) => total + service.receivedPayment,
        0
      );
      const pendingRecievedPaymentDate =
        remainingPaymentData.length > 0
          ? remainingPaymentData[0].paymentDate
          : null;
      ...
      if (serviceData) {
        ...
        console.error(
          `Service with name '${serviceName}' not found in selected company data.`
        );
      }
    });
    ...
    if (dataToSend.length !== 0) {
      ...
      try {
        ...
        const response1 = await axios.post(
          `${secretKey}/rm-services/postrmselectedservicestobookings/${selectedCompanyData["Company Name"]}`,
          {
            rmServicesMainBooking: primaryServiceNames || [],
            rmServicesMoreBooking: moreBookingServiceNames || [],
          }
        ...
        if (response1.status === 200) {
          ...
          const response2 = await axios.post(
            `${secretKey}/rm-services/post-rmservicesdata`,
            {
    ...
    if (shouldDisableButton) {
      ...
      try {
        const response = await axios.post(
          `${secretKey}/rm-services/postmethodtoremovecompanyfromrmpanel/${companyName}`,
          {
    ...
    <div>
                          ...
                          >
                            ...
                            <div className="d-flex justify-content-between align-items-center mt-1">
                              <div className="rm_bking_time">
                                ...
                                <span className="ml-1">
                                  ({numberToWords(activeIndexBooking)} Booking)
                                </span>
                              </div>
                                ...
                                {currentLeadform &&
                                  (currentLeadform.paymentReceipt.length !==
                                    ...
                                    <>
                                      <div className="mb-2 mt-3 mul-booking-card-inner-head">
                                        <b>
                                          Payment Receipt and Additional
                                          Documents:
                                        </b>
                                      </div>
                                      <div className="row">
                                        ...
                                        {currentLeadform.remainingPayments
                                          ...
                                          currentLeadform.remainingPayments.map(
                                            (remainingObject, index) =>
                                              remainingObject.paymentReceipt
                                                .length !== 0 && (
                                                <div
      ...
      >
        <DialogTitle>
          ...
          <IconButton
            onClick={handleCloseServicesPopup}
            style={{ float: "right" }}
          >
            <CloseIcon color="primary"></CloseIcon>
          </IconButton>{" "}
