...
// import { setRef, useIsFocusVisible } from "@mui/material";
...
//         socket.on("recruiter-general-status-updated", () => {
//             refetch();
//         });
//         socket.on("recruiter-application-submitted", () => {
//             refetch();
//         });
...
//     const fetchRMServicesData = async ({ queryKey }) => {
//         const [, page, activeTab] = queryKey;
...
//         return data; // Ensure this returns just the data, not the entire response
//     };

//     const { data: newData, isLoading, isError, refetch } = useQuery({
//         queryKey: ["recruiterData", page, activeTab],
//         queryFn: fetchRMServicesData,
...
//                 return (
//                     item.empFullName?.toLowerCase().includes(search) ||
//                     item.personal_email?.toLowerCase().includes(search) ||
//                     item.appliedFor?.toLowerCase().includes(search) ||
//                     item.applicationSource?.toLowerCase().includes(search) ||
//                     item.bdmName?.toLowerCase().includes(search) ||
//                     item.personal_number?.toString().includes(search) ||
//                     item.currentCTC?.toString().includes(search) ||
//                     item.expectedCTC?.toString().includes(search)
//                 );
//             });

//             setFilteredData(dataFiltered);

//             // Calculate counts for each category after filtering
//             setTotalDocumentsGeneral(dataFiltered.filter(item => item.mainCategoryStatus === "General").length);
//             setTotalDocumentsUnderReview(dataFiltered.filter(item => item.mainCategoryStatus === "UnderReview").length);
//             setTotalDocumentsOnHold(dataFiltered.filter(item => item.mainCategoryStatus === "On Hold").length);
//             setTotalDocumentsDisqualified(dataFiltered.filter(item => item.mainCategoryStatus === "Disqualified").length);
//             settotalDocumentsRejected(dataFiltered.filter(item => item.mainCategoryStatus === "Rejected").length);
//             settotalDocumentsSelected(dataFiltered.filter(item => item.mainCategoryStatus === "Selected").length);
//         }
//     }, [rmServicesData, search]);
