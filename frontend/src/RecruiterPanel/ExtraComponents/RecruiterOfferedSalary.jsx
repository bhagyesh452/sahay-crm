...
const RecruiterOfferedSalary = ({ empName, empEmail, offeredSalary ,refreshData , mainStatus}) => {
    const [charges, setCharges] = useState(offeredSalary);
    const secretKey = process.env.REACT_APP_SECRET_KEY;
    const [openEmailPopup, setOpenEmailPopup] = useState(false);

    const handleSubmitPortalCharges = async () => {
        try {
            const response = await axios.post(`${secretKey}/recruiter/post-save-offeredsalary`, {
                empName,
                empEmail,
                charges
            });
    ...
    return (
        <div>
            ...
            <div
                ...
                title={offeredSalary}
            >
                {!offeredSalary ? "Enter Offered Salary" : offeredSalary}
            </div>
...
export default RecruiterOfferedSalary;
